...
const COLLECTION = 'factures_fournisseurs'
...
export interface FactureFournisseur {
  ...
  // 🔄 MIGRATION - Origine de la facture
  origine: 'stock_flotte' | 'comptabilite'  // Où la facture a été créée
...
export interface FactureFournisseurInput {
  ...
  origine?: 'stock_flotte' | 'comptabilite'  // 🔄 MIGRATION - Optionnel, défaut = 'comptabilite'
  notes?: string
  createdBy: string
}
...
export async function createFactureFournisseur(data: FactureFournisseurInput): Promise<string> {
  try {
    ...
    const facture: FactureFournisseur = {
      ...
      origine: data.origine || 'comptabilite',  // 🔄 MIGRATION - Défaut 'comptabilite'
      mouvementsStockIds: [],
...
/**
 * 🔄 MIGRATION - Fonction compatibilité Stock & Flotte
 * Génère les mouvements stock pour une facture (appelle validerFactureFournisseur en interne)
 * Conserve le nom pour compatibilité avec l'ancien code Stock & Flotte
 */
export async function genererMouvementsStockFacture(
  ...
  try {
    // Appeler la fonction de validation qui génère mouvements + écritures
    await validerFactureFournisseur(factureId, operateur)
    
    // Récupérer la facture mise à jour
    const facture = await getFactureFournisseurById(factureId)
    if (!facture) {
      throw new Error('Facture introuvable après validation')
    }
    
    // Retourner les IDs des mouvements générés
    return facture.mouvementsStockIds || []
  } catch (error) {
