    ...
    try {
      if (suggestion.type === 'client') {
        await rapprocherAvecFactureClient(
          ...
          'Admin' // TODO: Remplacer par nom utilisateur connecté quand auth disponible
        )
      } else {
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-4 border-b">
            <h2 className="text-lg font-bold text-gray-900">
              {selectedLigne ? 'Rapprochement' : 'Sélectionner une ligne'}
            </h2>
          </div>

          {!selectedLigne ? (
            <div className="p-8 text-center text-gray-500">
              ← Sélectionner une ligne bancaire à gauche
            </div>
          ) : (
            <div className="p-4">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  <span className={`px-3 py-1 text-sm rounded font-medium ${
                    ...
                    {selectedLigne.montant > 0 ? 'CRÉDIT' : 'DÉBIT'}
                  </span>
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">
                  {selectedLigne.montant.toFixed(2)} €
                </p>
              </div>
                  ...
                  {searchTerm && (
                    <div className="mt-2 max-h-64 overflow-y-auto border rounded">
                      {searchResults.length === 0 ? (
                        <p className="p-4 text-gray-500 text-center">Aucun résultat</p>
                      ) : (
                        searchResults.map((item: any) => (
                          ...
                          >
                            ...
                            <p className="text-sm font-bold">{(item.totalTTC || item.montantTTC).toFixed(2)} €</p>
                          </div>
                        ))
                      )}
                    </div>
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs text-gray-600">
                                  {suggestion.type === 'client' ? '📤 Client' : '📥 Fournisseur'}
                                </span>
                              </div>
                              ...
                              <p className="text-sm text-gray-500 mt-1">
                                Montant: {suggestion.montant.toFixed(2)} €
                                {suggestion.difference > 0 && (
                                  <span className="text-red-600 ml-2">
                                    (Diff: {suggestion.difference.toFixed(2)} €)
                                  </span>
                                )}
                              </p>
                            </div>
