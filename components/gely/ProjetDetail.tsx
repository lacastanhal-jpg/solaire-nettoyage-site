...
import { PROJETS_MOCK, LIGNES_FINANCIERES_MOCK, calculerTotaux } from '@/lib/gely/mockData'
...
export default function ProjetDetail({ projetId, onBack }: ProjetDetailProps) {
  ...
  const handleAjoutLigne = (nouvelleLigne: LigneFinanciere, fichier?: File) => {
    setLignes([...lignes, nouvelleLigne])
    if (fichier) {
      const newMap = new Map(fichiersPDF)
      newMap.set(nouvelleLigne.id, fichier)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        <div className="bg-white rounded-xl shadow-lg p-6 border-t-4 border-yellow-500">
          ...
          <p className="text-2xl font-bold text-yellow-700">{projet.budgetTotalHT.toLocaleString('fr-FR')} € HT</p>
          <p className="text-sm text-gray-600">{projet.budgetTotal.toLocaleString('fr-FR')} € TTC</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border-t-4 border-blue-500">
          ...
          <p className="text-2xl font-bold text-blue-700">
            {(projet.budgetTotalHT - totaux.totalFacturesHT - totaux.totalDevisHT).toLocaleString('fr-FR')} € HT
          </p>
          <p className="text-sm text-gray-600">
            {(projet.budgetTotal - totaux.totalFactures - totaux.totalDevis).toLocaleString('fr-FR')} € TTC
          </p>
        </div>
        <div className="bg-gradient-to-br from-red-500 to-red-700 rounded-xl shadow-lg p-6 border-t-4 border-red-900">
          ...
          <p className="text-2xl font-bold text-white">
            {(projet.budgetTotalHT - totaux.totalPayeHT).toLocaleString('fr-FR')} € HT
          </p>
          <p className="text-sm text-white/80">
            {(projet.budgetTotal - totaux.totalPaye).toLocaleString('fr-FR')} € TTC
          </p>
        </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {lignes.map((ligne) => {
                  ...
                  return (
                    <tr key={ligne.id} className={`transition ${isEditing ? 'bg-blue-100 border-4 border-blue-600 bg-white text-black font-bold text-lg' : 'bg-white hover:bg-blue-200'}`}>
                      <td className="px-4 py-3">
                        {isEditing ? (
                          <select
                            value={displayLigne.type}
                            onChange={(e) => updateField('type', e.target.value)}
                            className="px-2 py-1 border-4 border-blue-600 bg-white text-black font-bold text-lg rounded w-full bg-white text-gray-900 font-semibold"
                          >
                      ...
                      <td className="px-4 py-3 text-right">
                        {isEditing ? (
                          <input
                            ...
                            className="px-2 py-1 border-4 border-blue-600 bg-white text-black font-bold text-lg rounded w-24 text-right bg-white text-gray-900 font-semibold"
                          />
                        ) : (
                          <span className="text-sm font-bold text-gray-900">{ligne.montantHT.toLocaleString('fr-FR')} €</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right">
                        {isEditing ? (
                          ...
                          <span className="text-sm font-bold text-gray-900">{ligne.montantTTC.toLocaleString('fr-FR')} €</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {isEditing ? (
                          <select
                            ...
                            className="px-2 py-1 border-4 border-blue-600 bg-white text-black font-bold text-lg rounded text-xs bg-white text-gray-900 font-semibold"
                          >
                          ...
                          {isEditing ? (
                            <>
                              <button
                                ...
                                className="p-1 bg-green-600 text-white border-2 border-black rounded hover:bg-green-200"
                                title="Sauvegarder"
                              >
                              ...
                              <button
                                ...
                                className="p-1 bg-gray-200 text-gray-900 rounded hover:bg-gray-200"
                                title="Annuler"
                              >
                                <X className="w-4 h-4" />
                              </button>
                            ...
                            <>
                              <button
                                ...
                                className="p-1 bg-blue-600 text-white border-2 border-black rounded hover:bg-blue-200"
                                title="Modifier"
                              >
                              ...
                              <button
                                ...
                                className="p-1 bg-red-600 text-white border-2 border-black rounded hover:bg-red-200"
                                title="Supprimer"
                              >
